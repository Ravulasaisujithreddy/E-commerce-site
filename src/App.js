...
function App() {
  ...
  const {filteredproducts,setfilterType,setSearch,isLoading}=useContext(FilterContext)
 ...
 const [displaySearch,setDisplaySearch]=useState(()=>{return location.pathname==='/'})
 useEffect(()=>{
  ...
  useEffect(()=>{
    ...
    setDisplaySearch(()=>{return location.pathname==='/'})
  })
  ...
  return (
    ...
    <div className={`min-h-screen ${theme === 'dark' ? 'bg-gray-800 text-white' : 'bg-gray-100 text-black'}`}>
            {showNavbar && <Navbar user={user} onLogin={handleLogin} setSearch={setSearch} displaySearch={displaySearch} handleSidebar={handleSidebar} />}
      <Routes>
        <Route path="/" element={isLoggedIn ? <Products products={filteredproducts} isLoading={isLoading} liked={liked} handleLike={handleLike} displaysidebar={displaysidebar}/> : <Navigate to="/login" />} />
        <Route path="/login" element={ <Login onLogin={handleLogin} /> } />
        <Route path="/product/:id" element={isLoggedIn ?<ProductDetails /> : <Navigate to="/login" />} />
        <Route path="/cart" element={isLoggedIn ?<Cart /> : <Navigate to="/login" />} />
        <Route path="/checkout" element={isLoggedIn ?<Checkout /> : <Navigate to="/login" />} />
        <Route path="/favourites" element={isLoggedIn ?<Favourites  favourites={favourites}/> : <Navigate to="/login" />} />
