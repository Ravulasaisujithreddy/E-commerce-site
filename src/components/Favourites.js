...
function Favourites({ favourites }) {
  ...
  if (!favourites.length) {
    return (
      <div
        className={`flex justify-center items-center h-[100vh] ${bgColor} ${textColor}`}
      >
    ...
    <div className={`p-6 ${bgColor} ${textColor} min-h-[100vh]`}>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        {favourites.map((product) => (
          <div
            key={product.id}
            className={`border rounded-lg overflow-hidden shadow-lg ${borderColor}`}
          >
            <Link to={`/product/${product.id}`} className="block">
              ...
              />
              <h3 className={`text-lg font-semibold p-2 ${textColor}`}>
                {product.title}
              </h3>
